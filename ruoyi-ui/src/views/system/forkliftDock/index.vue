<template>
  <div class="app-container">
    <el-form :model="queryParams" ref="queryForm" size="small" :inline="true" v-show="showSearch" label-width="68px">
      ...
      <el-form-item label="叉车编码" prop="forkliftCode">
        <el-input
    ...
    <el-table v-loading="loading" :data="dockList" @selection-change="handleSelectionChange">
      ...
      <el-table-column label="叉车编码" align="center" prop="forkliftCode" />
    ...
    <el-dialog :title="title" :visible.sync="open" width="500px" append-to-body>
      <el-form ref="form" :model="form" :rules="rules" label-width="80px">
        ...
        <el-form-item label="叉车编码" prop="forkliftCode">
          <el-input v-model="form.forkliftCode" placeholder="请输入叉车编码" />
        </el-form-item>
      </el-form>
...
<script>
import { listDock, getDock, delDock, addDock, updateDock } from "@/api/system/forkliftDock";
...
export default {
  ...
  methods: {
    ...
    handleAdd() {
      this.reset();
      this.open = true;
      this.title = "添加叉车-码头关联";
    },
    /** 修改按钮操作 */
    handleUpdate(row) {
      this.reset();
      const myRowId = row.myRowId || this.ids
      getDock(myRowId).then(response => {
        this.form = response.data;
        this.open = true;
        this.title = "修改叉车-码头关联";
      });
    },
    /** 提交按钮 */
    submitForm() {
    ...
    handleDelete(row) {
      ...
      this.$modal.confirm('是否确认删除叉车-码头关联编号为"' + myRowIds + '"的数据项？').then(function() {
        return delDock(myRowIds);
      }).then(() => {
        this.getList();
